...
export default function Home() {
  ...
  const { selectedAdmin, logoutAdmin, userList, removeUserFromList } = useContext(LoginContext);
  ...
  const [confirmationAnswer, setConfirmationAnswer] = useState<boolean>(false);
      ...
      <Box sx={{ flexGrow: 1, padding: 2 }}>
        <Grid container spacing={2}>
          <Grid item xs={12}>
            ...
            >
              <CustomButton onClick={() => setNewAdminModalOpen(true)}>Add new Admin</CustomButton>
            </Paper>
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            >
              <CustomButton onClick={() => setNewUserModalOpen(true)}>Add new User</CustomButton>
            </Paper>
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            >
              <CustomButton onClick={logoutAdmin}>Logout admin</CustomButton>
            </Paper>
          </Grid>
        </Grid>
      </Box>
  ...
  const adminPageLoggedIn = (
    <>
      <Typography sx={typographyStyle}>This is Admin page, welcome!</Typography>
      {adminButtons()}
    </>
  );
  ...
  return (
    ...
    >
      {iconRow()}
      {selectedAdmin != null ? adminPageLoggedIn : adminPageNotLoggedIn}
      {iconRow()}
                        ...
                        <Box display="flex" alignItems="center" flexDirection="row" justifyContent="space-between">
                          <Typography
                            ...
                            sx={{
                              ...
                              maxWidth: 'calc(100% - 100px)', // Adjust based on the delete button and padding
                            }}
                          >
                            {user.name}
                          </Typography>
      ...
      {newUserModalOpen && <NewUser onClose={() => setNewUserModalOpen(false)} />}
      {loginAdminModalOpen && <Login onClose={() => setLoginAdminModalOpen(false)} />}
      {confirmationModalOpen && <Confirmation question="Are you sure?" answer={(answer) => handleConfirmation(answer)} />}
    </Box>
  );
}
